...
document.addEventListener('DOMContentLoaded', () => {
  ...
  cntBtn.addEventListener('click', async () => {
    ...
    // إرسال للـ API
    const topic = document.querySelector('.cnt-chip.active')?.dataset.val || '';
    cntBtn.disabled = true;
    cntBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" style="animation:spin 1s linear infinite"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" stroke-dasharray="60" stroke-dashoffset="20"/></svg> جاري الإرسال...`;
    ...
    try {
      ...
      if (data.success) {
        ...
        document.querySelectorAll('.cnt-chip').forEach((c, i) => c.classList.toggle('active', i === 0));
      } else {
        if (cToast) {
          cToast.style.cssText = 'display:block;background:rgba(239,68,68,.16);border:1px solid rgba(239,68,68,.3);color:#f87171;';
          cToast.textContent = data.message || 'حدث خطأ، حاول مرة أخرى';
      ...
      if (cToast) {
        cToast.style.cssText = 'display:block;background:rgba(239,68,68,.16);border:1px solid rgba(239,68,68,.3);color:#f87171;';
        cToast.textContent = '⚠️ تعذر الاتصال بالخادم';
        setTimeout(() => { cToast.style.display = 'none'; }, 3000);
      }
    }

    cntBtn.disabled = false;
    cntBtn.innerHTML = `<i class="fas fa-paper-plane"></i> إرسال رأيك ←`;
  });
});
